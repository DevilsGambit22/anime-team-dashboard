(() => {
  ...
  function normalizeMatch(match, status) {
    const start = match.start_time || match.start_time_ts || match.start_time_timestamp;
  ...
  function setupMusic() {
    ...
    function loadTrack(index, autoplay = false) {
      ...
      if (autoplay) audio.play().catch(() => showToast("Press play to start audio"));
      renderPlaylist();
    }
    ...
    function renderPlaylist() {
      const list = $("#playlist");
      if (!tracks.length) {
        list.innerHTML = `<div class="empty-playlist">No tracks are configured. Add audio files to <code>assets/music</code> and list them in <code>config.js</code>.</div>`;
        return;
      }
      list.innerHTML = "";
      tracks.forEach((track, index) => {
        ...
        row.innerHTML = `<div><strong>${escapeHtml(track.title || "Untitled")}</strong><br><small>${escapeHtml(track.artist || "Unknown Artist")}</small></div>`;
    ...
    $("#playPause").addEventListener("click", () => {
      if (!tracks.length) return showToast("Add music files in config.js first");
      if (!audio.src) loadTrack(state.selectedTrack, false);
      if (audio.paused) audio.play().catch(() => showToast("Audio could not start"));
      else audio.pause();
    });
    $("#prevTrack").addEventListener("click", () => loadTrack(state.selectedTrack - 1, true));
    $("#nextTrack").addEventListener("click", () => loadTrack(state.selectedTrack + 1, true));
    ...
    audio.addEventListener("timeupdate", () => {
      ...
      $("#seekBar").value = Number.isFinite(audio.duration) && audio.duration > 0 ? (audio.currentTime / audio.duration) * 100 : 0;
    });
    audio.addEventListener("ended", () => loadTrack(state.selectedTrack + 1, true));
  ...
  function setupEasterEggs() {
    ...
    $("#footerEgg").addEventListener("click", () => {
      ...
      if (state.footerClicks >= 5) {
        showToast("DevilsGambit22 signature sequence activated");
        slashEffect();
        runScan();
        burst(80);
  ...
  function init() {
    ...
    $$(".theme-buttons button").forEach(btn => btn.addEventListener("click", () => setTheme(btn.dataset.themeChoice)));
    $$("[data-theme-jump]").forEach(btn => btn.addEventListener("click", () => setTheme(btn.dataset.themeJump)));
