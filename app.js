(() => {
  ...
  const state = {
    theme: ["onepiece", "jjk"].includes(localStorage.getItem("animeTeamTheme")) ? localStorage.getItem("animeTeamTheme") : "onepiece",
  ...
  function openPage(page) {
    ...
    if (page === "staff") fetchStaffProfiles();
  }
  ...
  function setupDragTabs() {
    ...
    // Keep native touch scrolling on phones/tablets. Desktop dragging uses
    // mouse events without pointer capture so normal button clicks still fire.
    ...
    window.addEventListener("mousemove", event => {
      ...
      if (!didDrag && Math.abs(delta) < 7) return;
      didDrag = true;
  ...
  async function fetchStaffProfiles() {
    ...
    const avatars = $$(".staff-avatar[data-username]");
    ...
    await Promise.all(avatars.map(async avatar => {
      ...
      try {
        ...
        console.warn(`[Anime Team] Staff profile unavailable for ${username}`, error);
      }
    }));
